...
> ⚠️ **Topes UVT en litigio, revisar periódicamente**: el Decreto 572/2025 bajó estos topes (servicios 4→2 UVT, compras 27→10 UVT); el Consejo de Estado lo suspendió el 8-may-2026 (volviendo a 4/27) y revocó esa suspensión el 2-jun-2026, reviviendo el decreto desde el 1-jul-2026 (2/10 de nuevo, los valores usados hoy como default).
>
> **`uvt_value` / `retention_threshold_parts_uvt` / `retention_threshold_services_uvt` dejaron de ser configurables por taller (2026-09-02, pedido explícito del usuario)** — antes el contador podía sobreescribirlos por taller desde el panel "Matriz de Retenciones (UVT)" (`ContadorPanel.jsx`), con una alerta ahí mismo recordando este litigio. Bug real corregido: el UVT y estos topes son un parámetro **legal único para todo el país** (DIAN/decreto), no una decisión de negocio de cada taller — no había ninguna razón legítima para que un taller tuviera un valor propio. Y en la práctica ninguno lo usaba para eso: en cuanto un contador guardaba CUALQUIER cosa en esa pantalla, el valor que tuviera en pantalla en ese momento quedaba **congelado** en su fila de `workshop_config` para siempre, sin volver a seguir el default de este archivo cuando la norma cambiara después. Verificado 2026-09-02 contra los 9 talleres reales de producción: 8 de 9 estaban reteniendo con el umbral viejo, de antes del Decreto 572/2025 (27/4 UVT en vez de 10/2) — un bug fiscal real y silencioso, no solo teórico.
>
> Fix: `ContadorPanel.jsx` pasó esa sección a solo lectura (muestra los 3 valores vigentes como texto, sin input), `workshop.controller.js` ya no acepta esos 3 campos en `updateWorkshop`/`updateFinanceSettings`, y la migración `2026-09-02_uvt_retention_thresholds_decreto572.sql` puso las 3 columnas en `NULL` para todos los talleres — con `NULL`, este archivo siempre gana (`config.x != null ? Number(config.x) : DEFAULT`, ver más abajo). **De ahora en adelante, la única forma de actualizar el UVT/topes es cambiar las constantes de este archivo (`UVT_VALUE`, `RETENTION_THRESHOLD_PARTS_UVT`, y el `: 2` inline de `serviceThresholdUvt` en `liquidateClientInvoice`) y desplegar** — nunca más una migración por taller, porque ya no hay forma de que un taller vuelva a congelar un valor propio.
>
> **Un décimo taller encontrado todavía congelado (2026-09-08)**: al auditar qué migraciones de `backend/scripts/migrations/` faltaban por correr contra producción, se encontró un taller (`c8009bf2-be25-44cf-9c82-ece062dc07eb`, activo y real, no de prueba) todavía con `uvt_value`/`retention_threshold_parts_uvt`/`retention_threshold_services_uvt` en los valores viejos — el `UPDATE` sin `WHERE` de la migración de arriba no lo alcanzó (probablemente creado o editado después de esa corrida). Corregido con el mismo `UPDATE ... SET ... = NULL`, esta vez acotado con `WHERE id = '...'` a solo ese taller — verificado después contra Supabase en vivo (vía REST con la service role key) que quedó en `NULL`, no solo asumido por el dump. Recordatorio de proceso: el `backend/backup/efiscodb.sql` (dump de esquema) confirma qué migraciones DDL ya corrieron, pero no migraciones puramente de DATOS como esta — esas solo se verifican contra la base en vivo, taller por taller si hace falta.
...
**`total_invoice` ahora se persiste, no solo se usa para armar el ledger (2026-09-13, pedido explícito del usuario)**: hasta entonces `financial_summary.total_invoice` (Total Factura de la fórmula de arriba) era puramente transitorio — se calculaba en cada `settleOrder` y quedaba repartido entre varias filas de `cash_flow_ledger`, sin ninguna columna propia que lo guardara entero. `settleOrder` ahora también lo escribe en `work_orders.invoice_total` (migración `2026-09-13_work_orders_invoice_total.sql`), justo después de calcular `results` — igual para contado y crédito, porque la venta se liquida por el Total Factura completo sin importar cómo se cobre después. Es lo que alimenta la columna "Monto" del Historial de Servicios, ver [OPERATIONS.md — Bahías](OPERATIONS.md#bahías-órdenes-de-trabajo).
...
**Modelo de datos**: columna nueva `workshop_config.retention_matrix_overrides` (jsonb, nullable — migración `2026-09-16_workshop_retention_matrix_overrides.sql`), mismo patrón que `category_vat_rates` (JSONB de overrides ya existente, ver [BUSINESS_RULES.md](BUSINESS_RULES.md#configuración-del-taller-config)): se guarda a través del mismo `PUT /api/workshop/:id` ya gateado a rol `contador` (`CONTADOR_FIELD_TRANSFORMS`), sin tabla ni ruta ni política RLS nuevas. Es **sparse**: solo existen las celdas que el contador de verdad tocó (`{ client: {"<rowKey>|<colKey>": {retefuente,reteica,reteiva}}, provider: {...}, gateway: {...} }`) — la ausencia de una clave es literalmente el indicador de "sigue en el default legal", no hace falta un campo `is_custom` aparte. "Restablecer a Valores por Defecto del Sistema" en la UI es un `PUT` con `retention_matrix_overrides: null`.
...
## 4. Desglose Efectivo vs Bancos (`calculateCashAccountBalances`)

Pedido explícito del usuario (2026-09-08): "que en finanzas también separes efectivo y banco" — `calculateGlobalHealth` (sección 3) da un solo `realBankBalance` combinado; esto responde una pregunta distinta: **cuánto de esa plata está físicamente en Caja vs cuánto en la cuenta bancaria**.

No hace falta ninguna columna nueva — se apoya en algo que ya existía: cada vez que el cajero elige `cash_channel` (`'efectivo'`/`'banco'`) al liquidar una orden, cobrar una cuota, vender un repuesto o pagar a un proveedor/mecánico, el asiento `CASH_RECEIPT` (entra) o `CASH_PAYMENT` (sale) ya se etiqueta con el `puc_code` de Caja o de Bancos según lo elegido (ver `settleOrder`/`payInstallment`/`sellDirect`/`registerPurchase`/`payMechanic`). La ironía es que esos dos `type` están en `NON_CASH_TYPES` (sección de abajo) — se excluyen de `calculateGlobalHealth` para no duplicar el ingreso/costo que ya cuenta el `INC_GROSS`/`SUP_PAY` hermano — pero son justo los que sí llevan tatuada la cuenta real (Caja o Bancos) a la que entró/salió la plata.

```
calculateCashAccountBalance(ledgerEntries, pucCode) =
  Σ (impact === 'DEBIT' ? +valor : −valor) de TODA fila con ese puc_code exacto

calculateCashAccountBalances(ledgerEntries, { cajaCode, bancosCode }) = {
  caja:   calculateCashAccountBalance(ledgerEntries, cajaCode),
  bancos: calculateCashAccountBalance(ledgerEntries, bancosCode)
}
```

A diferencia de `calculateGlobalHealth` (que filtra por `isCashEntry`/`type`), esto es literalmente el saldo de una cuenta T del Libro Mayor — suma **cualquier** fila etiquetada con ese `puc_code`, sin importar su `type`, con la convención estándar de partida doble (DEBIT aumenta un Activo, CREDIT lo disminuye). Incluye de paso `OPENING_BALANCE`/`CASH_RECEIPT`/`CASH_PAYMENT` de ese canal — cualquier asiento futuro que alguna vez se etiquete directo con Caja/Bancos también contaría, sin tocar esta función.

**No necesariamente `caja + bancos` cuadra con `realBankBalance`**: `realBankBalance` además resta el IVA pendiente a la DIAN (que no vive en ninguna de las dos cuentas), y algunos flujos (ingreso manual, ingreso por referido) siempre van a Bancos sin ofrecer canal — son vistas complementarias, no la misma cifra partida en dos.

**Consumido en**: `getDashboardSummary` (`financialMetrics.cashAccounts`, sobre el histórico completo del taller — mismo criterio que `realBankBalance`) y `getCashFlow` (`summary.cash_accounts`, sobre el histórico previo a `from` + el rango elegido — mismo criterio que `bank_balance`/`net_balance`). Ver [FINANCE.md — Dashboard](FINANCE.md#dashboard-dashboard) y [FINANCE.md — Flujo de Caja](FINANCE.md#flujo-de-caja-flujo-caja) para dónde se muestra. Cubierto por `tests/financialEngine.test.js` (suma/resta por cuenta, `CASH_RECEIPT`/`CASH_PAYMENT` contando aunque `calculateGlobalHealth` los excluya, `OPENING_BALANCE` por canal, `puc_code` ausente o ledger vacío sin explotar) y por `tests/finance/finance.dashboardSummary.today.test.js`/`finance.cashFlow.cashAccounts.test.js` a nivel de endpoint.

### Saldo Inicial por canal (2026-09-08, mismo pedido)

`setOpeningBalance` acepta ahora `channel: 'efectivo' | 'banco'` (default `'banco'`, compatible con cualquier llamador viejo que solo mande `amount` — mismos talleres que ya tenían su saldo inicial en `puc_bancos_code` siguen igual). El saldo "actual" para calcular la corrección (`delta`) se calcula **solo con las filas de ESE canal** (`puc_code = pucCode` del canal elegido) — Caja y Bancos llevan cada uno su propio historial de correcciones independiente, igual que dos cuentas distintas del Libro Mayor; corregir Bancos nunca lee ni toca el saldo de Caja, y viceversa. `getOpeningBalance` devuelve `cashAmount`/`bankAmount` por separado además del `currentAmount` combinado, agrupando el historial por `puc_code === puc_caja_code` (cualquier otro código, incluido uno legado si el contador cambió `puc_bancos_code` después de escribir esa fila, cae al bucket de banco — así una fila vieja nunca "desaparece" del total). Ver [BUSINESS_RULES.md](BUSINESS_RULES.md#configuración-del-taller-config) para dónde se configura (`OpeningBalanceModal.jsx` en el primer login, o "Corregir saldo inicial" en Ajustes) y `tests/finance/finance.setOpeningBalance.channel.test.js`.
...
| `SW_EXPENSE` | DEBIT | ✅ | Gasto de Software EFISCO al taller (2026-09-02) — `type` propio, distinto de `SUP_PAY`, usado solo por `admin.controller.js:uploadSoftwareInvoice` y su espejo `subscriptionBilling.service.js:recordWorkshopSideExpense`. Mismo motor de liquidación que `SUP_PAY` (`liquidateSupplierPurchase`), pero necesitaba un `type` distinto para poder estar en `COST_TYPES` (resta de Utilidad Neta) sin arrastrar también las compras normales de repuestos — sumar `SUP_PAY` ahí habría duplicado el costo con `INV_COGS` cuando ese repuesto se consuma. PUC default `5145` (Gastos), ya no cae a `puc_inventory_purchase_code` (bug real corregido el mismo día: antes casi siempre terminaba contabilizado como Inventario, `1435`, un activo, no un gasto) |

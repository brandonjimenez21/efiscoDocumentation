...
| Comunicaciones | Meta WhatsApp Cloud API + Telegram Bot API + Resend | Cascada de notificaciones/documentos (WhatsApp → Telegram → Email) — WhatsApp está `WHATSAPP_ENABLED=false` en producción desde 2026-08-18 (cuenta de Meta desactivada), así que hoy la cascada opera en la práctica como Telegram → Email; ver [ARCHITECTURE.md — Variables de Entorno](#variables-de-entorno) y `messagingChannel.service.js` |
...
# 'pendiente_pasarela' PARA SIEMPRE si el gateway es 'bold'/'addi' con
# mode:'online' — no hay integración real con Bold/Addi que dispare este
# webhook de vuelta hoy (aclarado 2026-09-05, ver SECURITY.md), así que la
# opción "Online (link)" del selector de pago queda atascada en la práctica.
# Ver nota de seguridad en SECURITY.md.)
...
# Resend (correos custom disparados por el backend — NO cubre el "Reset
# Password" del DUEÑO del taller, que sigue siendo un correo nativo de
# Supabase Auth con el SMTP configurado en el dashboard de Supabase). Usado
# para el correo de rechazo al eliminar un taller no activado (ver
# BILLING.md → Talleres), el "olvidé mi contraseña" de un EMPLEADO
# (mecánico/admin/contador — no vive en Supabase Auth, así que su reset SÍ
# pasa por Resend, ver SECURITY.md → Empleados no viven en Supabase Auth,
# 2026-08-24) Y, desde 2026-08-18, como tercer nivel de la cascada de
# mensajería al cliente (WhatsApp → Telegram → Email, ver arriba) —
# email.service.js expone sendMessage/sendDocument/
# sendEmployeePasswordResetEmail con el mismo contrato { success, ... } que
# whatsapp.service.js/telegram.service.js. Sin esta key,
# emailService.isConfigured() es false y esos flujos simplemente no se
# intentan (mismo criterio fail-soft que los otros dos canales).

...
| POST | `/api/auth/login` | Pública | Login (dueño, empleado o contador) — rate-limit de 10 intentos fallidos por `(IP, email)` cada 15 min (429), ver [SECURITY.md](SECURITY.md#endurecimiento-de-seguridad-2026-07-19). Setea la cookie httpOnly `efisco_token` (ver [Migración a cookies httpOnly](SECURITY.md#migración-de-autenticación-a-cookies-httponly-2026-07-21)); el body ya no incluye el token, solo `user` y `expiresAt`. Para un empleado, `user` incluye `can_manage_catalog`/`available_as_mechanic` (2026-08-24 — antes solo `GET /me` los traía, así que un admin recién logueado no veía "Ajustes" hasta refrescar) y `mechanics_can_access_reception` (2026-09-13, leído de `workshop_config` — gatea Recepción/Cotizaciones para el rol mecánico, ver [SECURITY.md](SECURITY.md#recepción-y-cotizaciones-restringidas-al-mecánico-por-defecto-2026-09-13)) |
| POST | `/api/auth/logout` | Pública | Limpia la cookie `efisco_token` |
| GET | `/api/auth/me` | Taller | Restaura la sesión al recargar la página (la cookie httpOnly ya no se puede leer/decodificar en el cliente) — devuelve el mismo `user` rico que `login()`, exento de los gates de contrato/autorización del taller. Para un empleado, ahora también consulta `workshop_config` para traer `mechanics_can_access_reception` (2026-09-13 — antes esta rama no tocaba `workshop_config` en absoluto), así un mecánico ve reflejado sin cerrar sesión que el dueño le acaba de habilitar Recepción/Cotizaciones |
...
| GET | `/api/work-orders/history` | Taller (incl. `contador`, sidebar agregado 2026-09-13 — el endpoint en sí nunca distinguió por este rol) | Historial de órdenes completadas (incl. `mechanics_names[]`/`services_names[]`/`parts_used[]` — repuesto, cantidad, lado/posición y proveedor si se eligió uno, agregado 2026-07-23; `tools_names[]` agregado 2026-08-01; `client_email` agregado 2026-08-18, join a `clients` por `client_cedula`, `null` si el cliente nunca completó el registro público; para rol mecánico filtra por sus filas en `work_order_mechanics` + el escalar legacy). `select('*')` sobre `work_orders` incluye `invoice_total`/`payment_method` (2026-09-13, ver [FINANCIAL_ENGINE.md](../Reglas%20de%20Negocio%20y%20Finanzas/FINANCIAL_ENGINE.md#1-liquidación-de-servicios-liquidateclientinvoice)) — `null`/`undefined` en órdenes liquidadas antes de esa fecha. Acepta `from`/`to` (rango sobre `end_time`) y `limit`/`offset` (default 50, tope 100) — respuesta `{ data, total }`, no un array plano (fix 2026-07-19, antes traía todo el historial sin límite, ver [OPERATIONS.md](../Reglas%20de%20Negocio%20y%20Finanzas/OPERATIONS.md#bahías-órdenes-de-trabajo)) |
...
> **"Taller" en esta sección y en "Quotes / Cotizaciones" sigue significando cualquier empleado autenticado del taller, sin excepción de rol** — a diferencia de la pantalla que los consume (2026-09-13, `mechanics_can_access_reception` restringe Recepción/Cotizaciones para el rol mecánico en `Sidebar.jsx`/`CommandPalette.jsx`/las rutas de `App.jsx`), ninguno de estos endpoints tiene chequeo de rol en el backend. Deliberado por ahora: varias de estas mismas rutas (`GET /ready`, `POST /:id/claim`, `PATCH /:id/plate`, `GET /quotes/by-intake/:id`, `GET /quotes/by-work-order/:id`) las sigue necesitando Bahía para CUALQUIER mecánico, con o sin ese flag — ver [SECURITY.md](SECURITY.md#recepción-y-cotizaciones-restringidas-al-mecánico-por-defecto-2026-09-13).
...
| Método | Ruta | Auth | Descripción |
|:---|:---|:---:|:---|
| GET | `/api/quotes/?search=` | Taller | Listar cotizaciones del taller (las 3 columnas del tablero se filtran en el frontend por `status`). **Sin `search`** (2026-09-12, tope bajado de 30 días a 24h el 2026-09-15): pendientes/aprobadas completas, pero rechazadas solo las de las últimas 24h (`RECHAZADAS_HORAS_DEFAULT`, `quotes.controller.js`) — Rechazada es un estado terminal que se acumula para siempre, a diferencia de los otros dos; ver [OPERATIONS.md — Cotizaciones](../Reglas%20de%20Negocio%20y%20Finanzas/OPERATIONS.md#cotizaciones-cotizaciones). **Con `search`** (placa o nombre de cliente, `ilike`): ese tope se levanta y busca en todo el historial de rechazadas. `.limit(500)` como tope de seguridad en ambos casos. Cada cotización con `work_order_id` trae además `bahia_parts[]` (2026-09-10) — todo lo que el mecánico agregó en "Repuestos" de esa orden después de cotizar (nombre/cantidad/precio/`created_at`/`from_quote_suggestion`/`vat_percentage`; `from_quote_suggestion`, 2026-09-11 — `true` si esa fila viene de aceptar el banner de "repuesto ya cotizado" en Bahía, no un repuesto nuevo, el frontend la excluye del total para no cobrarla dos veces; `vat_percentage`, 2026-09-12 — el IVA de ese repuesto ya congelado al momento de agregarlo, usado por el desglose de IVA del bloque "Agregado en Bahía"; ambos ver [OPERATIONS.md](../Reglas%20de%20Negocio%20y%20Finanzas/OPERATIONS.md#bahías-órdenes-de-trabajo)), para el bloque "Agregado en Bahía" de la tarjeta. Cada cotización sin `work_order_id` todavía trae además `intake_status` (2026-09-11) — el `status` del intake asociado, usado por el frontend para saber si ya se envió a Bahía aunque la orden real no se haya creado todavía (ver [OPERATIONS.md](../Reglas%20de%20Negocio%20y%20Finanzas/OPERATIONS.md#cotizaciones-cotizaciones)). El frontend hace polling de este endpoint cada ~18s mientras `/cotizaciones` está abierta |
| GET | `/api/quotes/history?limit=&offset=&search=&status=&from=&to=` | Taller | (2026-09-15) Historial completo de cotizaciones, paginado (`{ data, total }`), **sin** el tope de 24h que sí aplica el endpoint de arriba a las rechazadas — cualquier estado, cualquier antigüedad. `status` filtra por `pendiente`/`aprobada`/`rechazada`; `from`/`to` filtran por `created_at` (mismo criterio de zona horaria Bogotá que `GET /api/work-orders/history`, `asBogotaIso`). Vive en `/ordenes` (`Historial.jsx`) como pestaña "Cotizaciones", reusando el mismo buscador y rango de fechas de la pestaña "Órdenes" en vez de un buscador/date-picker propio — ver [OPERATIONS.md — Cotizaciones](../Reglas%20de%20Negocio%20y%20Finanzas/OPERATIONS.md#cotizaciones-cotizaciones) |
